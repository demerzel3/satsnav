...
//        Timer.scheduledTimer(withTimeInterval: self.waitTime!, repeats: false) { _ in
...
//        }
    ...
    private func checkKeys(data: Data) {
        ...
        self.resultData.append(data)
        str.removeLast()
        let suf = String(str.suffix(self.lastKey!.count))
        if suf == self.lastKey! {
            if let completion = self.completion {
                self.resultData.removeLast()
                completion(self.resultData)
    ...
    private func startReceive() {
        self.connection.receive(minimumIncompleteLength: 1, maximumLength: 65536) { data, _, _, error in
            print("did receive something", data)
    ...
    public func sendJsonRpc<T: Codable>(input: Codable, output: T.Type) async throws -> T {
        guard self.lastKey != nil else { throw TCPError.jsonRpcIsNotActive }
    ...
    public func send(data: Data, completion: @escaping Completion) {
        guard self.completion == nil else {
            print("completion ist nicht nil")
            fatalError()
        }
